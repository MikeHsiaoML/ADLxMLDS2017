...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.v_ind = torch.LongTensor(4).fill_(0).unsqueeze(0)
        self.a_ind = torch.LongTensor(np.arange(1, 5)).unsqueeze(0)
    ...
    def forward(self, x):
        ...
        x = self.linear2(x)
        v_ind_vb = Variable(self.v_ind).cuda()
        a_ind_vb = Variable(self.a_ind).cuda()
        v = x.gather(1, v_ind_vb.expand(x.size(0),4))
        a = x.gather(1, a_ind_vb.expand(x.size(0),4))
        x = v + (a - a.mean(1).unsqueeze(1).expand(x.size(0), 4))
...
class Agent_DQN(Agent):
    ...
    def init_game_setting(self):
        ...
        # YOUR CODE HERE #
        ##################
        checkpoint = torch.load('./save/dqn/double/3200000.tar')
        self.Q = DQN().cuda()
    ...
    def optimize_model(self):
        ...
        next_state_values = self.target_Q(non_final_next_states).detach()
        ...
        # requires_grad=False
        max_action_next = self.Q(non_final_next_states).detach().max(1)[1].data
        index = torch.arange(0,float(len(max_action_next))).type(LongTensor)
        chosen_next_state_values = Variable(torch.zeros(self.batch_size).type(Tensor))
        chosen_next_state_values[non_final_mask] = next_state_values[index,max_action_next]

        chosen_next_state_values.volatile = False
        
        # Compute the expected Q values
        expected_state_action_values = (chosen_next_state_values * self.gamma) + reward_batch
